...
function show_open_spots(object_type, ignore_id) {
  ...
  if (has_resources(object_type, ignore_id)) {
    ...
    if (object_type == "settlement") {
      ...
      if (turn_actions.length == 0 || node_to_ignore == turn_actions[0].action_data || current_game.round_num >
        2) {
        //  Setup mode: Show all valid build spots on the board
        $(".buildspot:not(.locked)")
          .hide();
        $(".buildspot:not(.locked)")
          .each(function() {
            //  Find the node in the nodes object based on the id of this object
            var node_id = parseInt($(this)
              .attr('id')
              .replace("node_", ""));
...
function set_object_on_canvas(event, ui) {
  ...
  var object_type = (object_dragged_id.indexOf("settlement") > -1 ? "settlement" : (object_dragged_id.indexOf(
    "road") > -1 ? "road" : "city"));
  ...
  var node_id = parseInt(node_on_canvas.attr("id")
    .replace("road_", "")
    .replace("node_", ""));
  ...
  if (!can_build_this || (node.owner > -1 && object_type != "city") || (object_type == "city" && node.owner != current_game.player.id) || (object_type == node.building)) {
    ...
    $("#" + object_dragged_id)
      .attr("id", dragged_object_new_id);

    //  If this is a road, we might need to adjust the angle
    if (object_type == "road") {
      var classes = node_on_canvas.attr('class')
        .split(' ');
      for (var i = 0; i < classes.length; i++) {
        if (classes[i].indexOf("angle") > -1) {
          var object_class = object_dragged.attr('class')
            .replace("angle30", "")
            .replace("angle90", "")
            .replace("angle330", "");
    ...
    if (object_dragged.attr("data-card-list")) {
      resource_list = (object_dragged.attr("data-card-list")
        .split(","));
    }

    //  Is a road building card going to effect this action?
    var using_road_building_now = current_player.road_building_used && current_player.free_roads > 0 &&
      object_type == "road";
...
function reset_node_on_board(object_dragged_id, object_type) {
  ...
  var last_node_id = parseInt(object_dragged_id.replace(object_type + "_" + current_player.colour +
    "_pending_", ""));
  ...
  var last_canvas_node_class = $("#" + (object_type == "road" ? "road" : "node") + "_" + last_node_id)
    .attr("class");
...
function return_object(object_to_return, object_to_return_id, last_node_id, clear_node) {
  ...
  if (last_node_id > -1) {
    //  Get the type of structure
    var object_type = (object_to_return_id.indexOf("settlement") > -1 ? "settlement" : (object_to_return_id.indexOf(
      "road") > -1 ? "road" : "city"));
    ...
    //  Reset class
    object_to_return.attr('class', object_type + ' ' + current_player.colour + ' ' + (object_type == "road" ?
      "angle30 " : "") + 'ui-draggable ui-draggable-handle');
  ...
  for (var i = 1; i < turn_actions.length; i++) {
    ...
    if (node.id != next_object_node.id || next_object_type != object_type) {
      if (!has_valid_path(next_object_type, next_object_node, "")) {
        ...
        var object_to_return = $("#" + next_object_type + "_" + current_player.colour + "_pending_" +
          next_object_node.id);
...
function rotateRoad(event) {
  ...
  //  Swap out class
  event.toElement.className = event.toElement.className.replace("angle0", "angle" + angle)
    .replace("angle30", "angle" + angle)
    .replace("angle90", "angle" + angle)
    .replace("angle330", "angle" + angle)
...
function stash_node(object_type, new_node, old_node) {
  ...
  if (object_type == "settlement") {
    new_node.building = old_node.building;
  }

  nodes_to_use[old_node.id].owner = -1;
  nodes_to_use[old_node.id].status = "";
  if (object_type == "settlement") {
    nodes_to_use[old_node.id].building = "";
  }
}
...
function restore_node(object_type, new_node, old_node) {
  ...
  if (object_type == "settlement") {
    nodes_to_use[new_node.id].building = new_node.building;
  }
}